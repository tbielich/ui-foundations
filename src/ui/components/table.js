...
function getSelectableRows(table) {
  return Array.from(table.querySelectorAll(ROW_SELECTOR)).filter((row) =>
    isSelectableRow(table, row),
  );
}
...
function syncHeaderCheckbox(table) {
  if (getSelectionMode(table) !== "multi") return;

  const checkbox = table.querySelector(`thead ${CHECKBOX_SELECTOR}`);
  if (!checkbox) return;

  const selectableRows = getSelectableRows(table);
  const selectedRows = getSelectedRows(table);
  checkbox.checked = selectableRows.length > 0 && selectedRows.length === selectableRows.length;
  checkbox.indeterminate = selectedRows.length > 0 && selectedRows.length < selectableRows.length;
}
...
function isSelectableRow(table, row) {
  ...
  if (selectionMode === "multi") {
    return Boolean(row.querySelector(CHECKBOX_SELECTOR));
  }
...
function selectAllRows(table, selected) {
  getSelectableRows(table).forEach((row) => {
  ...
  table.dispatchEvent(
    new CustomEvent("uif:select", {
      bubbles: true,
      detail: { selectedRows: getSelectedRows(table) },
    }),
  );
}
...
function flushResizeFrame() {
  ...
  resizeState.header.style.setProperty("--runtime-table-col-width", `${nextWidth}px`);
...
function enhanceTableElement(table) {
  ...
  table.addEventListener("click", function (event) {
    ...
    if (checkbox && checkbox.closest("thead") && getSelectionMode(table) === "multi") {
      selectAllRows(table, checkbox.checked);
      return;
    }
