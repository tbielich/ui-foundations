...
export async function implementComponentHandler(
  ...
  const lines: string[] = [
    ...
    `### 3. React Wrapper`,
    ``,
    `File: \`src/react/${name}.js\``,
    ``,
    `- Named \`export function\` (PascalCase component name)`,
    `- Use \`React.createElement\` — no JSX`,
    `- No CSS imports (styles come from the pattern CSS)`,
    `- Forward refs and spread remaining props`,
    ...
    `- Test React wrapper produces equivalent output`,
    ``,
    `### 10. Accessibility`,
    ``,
    `Requirements:`,
    ...
    `| React wrapper | \`src/react/${name}.js\` |`,
